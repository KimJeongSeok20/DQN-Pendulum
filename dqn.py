...
class DQNAgent:
    ...
    def get_action(self, state):
        if np.random.rand() <= self.epsilon:
            ...
            discrete_action = np.argmax(q_value[0])
            
        return -2 + (4 / (self.action_size - 1)) * discrete_action

    # 샘플 <s, a, r, s'>을 리플레이 메모리에 저장
    def append_sample(self, state, action, reward, next_state, done):
        discrete_action = int((action + 2) * (self.action_size - 1) / 4)
        self.memory.append((state, discrete_action, reward, next_state, done))
